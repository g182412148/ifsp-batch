...
@Slf4j
@Service("aliLocalTxnService")
public class AliLocalTxnServiceImpl implements GetLocalInfoService {
    ...
    public CommonResponse getLocalTxnInfo(GetOrderInfoRequest req) {
         ...
         * 抽取数据(表复制)
         */
        int count = aliBillLocalDao.copy(txnDate, recoDate);
        log.info("支付宝数据抽取订单数量:" + count);
    ...
    private void clear(Date recoDate) {
        ...
        int clearCount = aliBillLocalDao.clear(recoDate);
        log.info("删除对账日下支付宝订单数量:" + clearCount);
