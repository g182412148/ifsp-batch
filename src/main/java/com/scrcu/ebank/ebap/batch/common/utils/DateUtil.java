...
public class DateUtil {
	...
	public static final String	dtLong				= "yyyyMMddHH:mm:ss";
	...
	public static String formatToTime(Date date, String pattern) {
		...
		String currentTime = dateStr+hour+minute+seconds;
		
		SimpleDateFormat sdf1 = new SimpleDateFormat("yyyyMMddHHmmss");
		SimpleDateFormat sdf2 = new SimpleDateFormat(pattern);
		try {
			Date currentDate  = sdf1.parse(currentTime);
	 ...
	 * @return
	 */
	public static final Date getDiffDate(int diff, int minute) {
		Calendar c = Calendar.getInstance();
		c.setTime(new Date());
		c.add(Calendar.DATE, diff);
		c.add(Calendar.MINUTE, minute);
	...
	public static Date getDiffStringDate(Date dt, int diff) {
...
//		System.out.println(getDiffDateTime(0, -30));
//	}
	...
	public static String getNextMonthDay(int day) {
		Calendar cal = Calendar.getInstance();
	...
	public static String getNMonthLater(int n)
	{
	    ...
	    SimpleDateFormat dft = new SimpleDateFormat("yyyyMMdd");
	    String nMonthLater = dft.format(cal.getTime());
	    return nMonthLater;
	}
	...
	public static Date getDate(String yyyyMMdd){
		return DateTimeFormat.forPattern(IfspDateTime.YYYYMMDD).parseDateTime(yyyyMMdd).toDate();
	}
	 ...
	 * @return
	 */
	public static Date getBeforeDate(String yyyyMMdd, int i){
		return DateTimeFormat.forPattern(IfspDateTime.YYYYMMDD).parseDateTime(yyyyMMdd).minusDays(i).toDate();
	}
	 ...
	 * @return
	 */
	public static Date getAfterDate(String yyyyMMdd, int i){
		return DateTimeFormat.forPattern(IfspDateTime.YYYYMMDD).parseDateTime(yyyyMMdd).plusDays(i).toDate();
	}
	...
	public static String toDateString(Date date){
		return new DateTime(date).toString(Constans.dateFormat);
	}
	 ...
	 * @return
	 */
	public static boolean checkDateTime(String dateTime,String pattern){

		if(IfspDataVerifyUtil.isBlank(dateTime)){
			return false;
		}
		if(IfspDataVerifyUtil.isBlank(pattern)){
			return false;
		}
		if(dateTime.length()!=pattern.length()){
			return false;
		}
		SimpleDateFormat sdf = new SimpleDateFormat(pattern);
		String dateTimeFormat = "";
		try {
			Date dateTimeTmp = sdf.parse(dateTime);
			dateTimeFormat = sdf.format(dateTimeTmp);
		} catch (Exception e) {
			e.printStackTrace();
			return false;
		}
		if(!dateTime.equals(dateTimeFormat)){
			return false;
		}
		return true;
	}
