 ...
 * 说明：<br>
 */
@Service("aliClearingServiceImp")
@Slf4j
public class AliClearingServiceImp implements OrderClearing
{
    ...
    public CommonResponse channelClearing(BatchRequest request) throws Exception
    {
        ...
        initPool();
        aliClearingExecutor.init(request.getPagyNo(),batchDate);
        ...
        try
        {
            ...
            while(true)
            {
                ...
                for(Iterator<BthMerInAccDtl> it=clearDataList.iterator();it.hasNext();)
                {
                    ...
                    {
                        ClearingTask clearingTask = new ClearingTask(subList, aliClearingExecutor,countDownLatch);
...
//                countDownLatch.await(5, TimeUnit.MINUTES);
            ...
            for (Future future : futureList)
            {
                ...
                {
                    log.error("支付宝清分线程处理异常: ", e);
                    //返回结果
                    commonResponse.setRespCode(SystemConfig.getSysErrorCode());
                    commonResponse.setRespMsg("支付宝清分线程处理异常:" + e.getMessage());
                }
            }
            clearDataList.clear();
...
//            log.info("批量清分明细表完成，数量【{}】，耗时【{}】",count,System.currentTimeMillis()-start);
        ...
        finally
        {
            //释放线程池
            destoryPool();
            log.info("支付宝清分处理完成，总耗时【{}】",System.currentTimeMillis()-start);
        }
    ...
    class ClearingTask<T> implements Callable<T>
    {
        ...
        ClearingExecutor aliClearingExecutor;
        private CountDownLatch countDownLatch;
        public ClearingTask(List<BthMerInAccDtl> clearDataList, ClearingExecutor aliClearingExecutor,CountDownLatch countDownLatch)
        {
            this.clearDataList = clearDataList;
            this.aliClearingExecutor = aliClearingExecutor;
...
//            }
            ...
            aliClearingExecutor.execute(this.clearDataList);    //根据订单编号（分片批量）进行清分
            clearDataList.clear();
...
//			}
    ...
    private void initPool()
    {
        ...
        executor = Executors.newFixedThreadPool(threadCount, new ThreadFactory()
        {
            ...
            {
                return new Thread(r, "aliClearingHander_" + this.atomic.getAndIncrement());
            }
        });
        log.info("====初始化线程池(end)====");
    }
