...
@Service("wxClearingExecutor")
@Slf4j
public class WxClearingExecutor implements ClearingExecutor
{
    ...
    @Value("${wxSubject}")
    private String subject;                     //微信科目   "94610114";
    @Value("${wxSeqNo}")
    private String seqNo;                       //微信内部帐序号 00000
...
//        bthSetCapitalDetailDao.delete("clearCapitalDetailTemp",params);
    ...
    public void execute(List<BthMerInAccDtl> inAccDtlList) throws Exception
    {
        ...
        List<String> subMchtIdList = inAccDtlList.stream().filter(x -> x.getChlSubMerId()!=null).map(BthMerInAccDtl::getChlSubMerId).filter(
                Objects::nonNull).collect(Collectors.toList());   //生成二级商户号为泛型的List
        ...
        for(BthMerInAccDtl inAccDtl:inAccDtlList)
        {
            ...
            //二级商户直接结算给本商户
            if (!IfspDataVerifyUtil.isBlank(inAccDtl.getChlSubMerId()) && (Constans.STL_TO_MER.equals(
                    inAccDtl.getStlType())
...
//        txnSeqIdList.clear();
    ...
    public void execute(BthMerInAccDtl inAccDtl) throws Exception
    {
        ...
        {
            log.info("微信订单清分出现异常_[{}]", e.getMessage());
...
//			bthSetCapitalDetailDao.insertSelective(detail);
        ...
        else if (Constans.ORDER_TYPE_RETURN.equals(settleInfo.getOrderType()))
        {
            ...
            {
                detail.setTranAmount(merIncome.multiply(MINUS_ONE));  //退款记负数
...
//				bthSetCapitalDetailDao.insertSelective(detail);
            ...
            else if (Constans.REFUND_ACC_TYPE_GUARANTEE.equals(settleInfo.getRefundAccType())&&Constans.SETTL_ACCT_TYPE_PLAT.equals(merSettleInfo.getSettlAcctType()))
            {
                ...
                //交易金额
                detail2.setTranAmount(merIncome.multiply(MINUS_ONE));
...
//				bthSetCapitalDetailDao.insertSelective(detail);
            ...
            else if (Constans.REFUND_ACC_TYPE_SETTLE.equals(settleInfo.getRefundAccType())||(Constans.REFUND_ACC_TYPE_GUARANTEE.equals(settleInfo.getRefundAccType())&&Constans.SETTL_ACCT_TYPE_CROSS.equals(merSettleInfo.getSettlAcctType())))
            {
                ...
                BigDecimal diffAmt = new BigDecimal(settleInfo.getSetlFeeAmt()).add(
                        new BigDecimal(settleInfo.getLogisFee())).add(new BigDecimal(settleInfo.getCommissionAmt()));
...
//    @Transactional(propagation = Propagation.REQUIRED, rollbackFor = {Exception.class})
    public void assignMerFee(BthMerInAccDtl settleInfo, MchtBaseInfo merInfo,
                             MchtContInfo merSettleInfo, MchtGainsInfo gainsInfo,
                             List<BthSetCapitalDetail> bthSetCapitalDetails,ParternBaseInfo serviceInfo)
    {
        ...
        if (feeIncome.compareTo(ZERO) == 1)
        {
            ...
            //收单机构分润比例
            if (gainsInfo.getCellectOrgDist() != null && gainsInfo.getCellectOrgDist().compareTo(
                    WxClearingExecutor.ZERO) == 1)
            {
            ...
            //运营机构分润比例/运营机构可能为空..
            if (gainsInfo.getProxyOrgDist() != null && gainsInfo.getProxyOrgDist().compareTo(
                    WxClearingExecutor.ZERO) == 1 && (merInfo.getOpOrgNo() != null))
            {
...
//    @Transactional(propagation = Propagation.REQUIRED, rollbackFor = {Exception.class})
    public void addSubsidyDetail(BthMerInAccDtl settleInfo, MchtBaseInfo merInfo, MchtContInfo merSettleInfo,
                                 BigDecimal subSidyAmt, String entryType,
...
//    @Transactional(propagation = Propagation.REQUIRED, rollbackFor = {Exception.class})
                              ...
                              BigDecimal channelFee, String entryType, List<BthSetCapitalDetail> bthSetCapitalDetails)
    {
        ...
        if (Constans.ORDER_TYPE_CONSUME.equals(settleInfo.getOrderType()))
        {
            /***
             * 补渠道手续费
             * 借：商户待清算账户            退款金额*微信通道手续费比例；
             * 贷：微信清算账户               退款金额*微信通道手续费比例；
             *
             * **/
            ...
            //转出账户:微信清算账户
            detail.setInAccountNo(
                    AccountUtil.genInnerAccountWithSeq(merInfo.getRpsOrgNo(), this.getSubject(), this.getSeqNo()));
...
//		bthSetCapitalDetailDao.insertSelective(detail);
    ...
    private BthSetCapitalDetail initCapitalDetail(BthMerInAccDtl settleInfo, MchtBaseInfo mgtMerInfo, String cur)
    {
        ...
        String timestampSecond = DateUtil.format(d, "yyyyMMddHHmmss");
        ...
        detail.setCreateDate(timestampSecond);                             // 创建时间
