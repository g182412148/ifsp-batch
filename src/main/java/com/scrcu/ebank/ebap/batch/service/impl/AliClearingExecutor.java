...
@Service("aliClearingExecutor")
@Slf4j
public class AliClearingExecutor implements ClearingExecutor
{
    ...
    @Value("${aliSubject}")
    private String subject;                     //支付宝科目
    @Value("${aliSeqNo}")
    private String seqNo;                       //支付宝内部帐序号
...
//        bthSetCapitalDetailDao.delete("clearCapitalDetailTemp",params);
    ...
    public void execute(List<BthMerInAccDtl> inAccDtlList) throws Exception
    {
        ...
        {
            count=this.updateOrderStlStatus(txnSeqIdList);
        }

        log.info("当片订单更新明细表完成，数量【{}】耗时【{}】",count,System.currentTimeMillis()-start);
        //批量插入清分表
        if (bthSetCapitalDetails.size() > 0)
        {
            count = bthSetCapitalDetailDao.insertBatch(bthSetCapitalDetails);
...
//    @Transactional(propagation = Propagation.REQUIRED, rollbackFor = {Exception.class})
    public void addCapitalDetail(BthMerInAccDtl settleInfo, MchtBaseInfo mgtMerInfo,
                                  MchtContInfo merSettleInfo, String cur,
                              ...
                              List<BthSetCapitalDetail> bthSetCapitalDetails,ParternBaseInfo serviceInfo)
    {
        ...
        if (feeIncome.compareTo(ZERO) == 1)
        {
            ...
            if (gainsInfo.getProxyOrgDist() != null && gainsInfo.getProxyOrgDist().compareTo(
                    AliClearingExecutor.ZERO) == 1 && (merInfo.getOpOrgNo() != null))
            {
...
//		bthSetCapitalDetailDao.insertSelective(detail);
    ...
    private BthSetCapitalDetail initCapitalDetail(BthMerInAccDtl settleInfo, MchtBaseInfo mgtMerInfo, String cur)
    {
        ...
        detail.setUpdateDate(IfspDateTime.getYYYYMMDDHHMMSS());
        //初始化批次号：日期加商户号
