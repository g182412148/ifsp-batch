...
@Service
@Slf4j
public class LoanpayOrderChkServiceImpl implements LoanpayOrderChkService 
{
	...
	@Override
	public CommonResponse loanpayOrderChk(BatchRequest request) throws Exception 
	{
		log.info(">>>>>>>>>>>>>>>>>>>>>loanpayOrderChk executing..." );
		...
		params.put("pagySysNo", Constans.LOAN_SYS_NO+" ");
		params.put("chkSuccDt", DateUtil.format(new Date(), "yyyyMMdd"));     //支付渠道： 06-授信支付
	...
	private int getTotalResult()
	{
		...
		params.put("fundChannel", Constans.GAINS_CHANNEL_LOAN);     //支付渠道： 06-授信支付
		params.put("orderState", Constans.ORDER_STATUS_SUCC);          
		count = paySubOrderInfoDao.count("countLoanpayDataByDate", params);
		log.info(">>>>>>>>>>>>>>>>>>>>>>>>>>>loan order count is : " + count);
		return count;
	}
	...
	private List<PayOrderInfo> getDataList(int beginIndex,int endIndex)
	{
		...
		List<PayOrderInfo> orderList = payOrderInfoDao.selectList("selectLoanpayDataByDate", params);
	...
	private BthChkRsltInfo initChkRsltInfo(PayOrderInfo order)
	{
         ...
         record.setPagySysNo(Constans.LOAN_SYS_NO);
