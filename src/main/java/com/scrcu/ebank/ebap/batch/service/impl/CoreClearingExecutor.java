...
@Service("coreClearingExecutor")
@Slf4j
public class CoreClearingExecutor implements ClearingExecutor {
...
//        txnSeqIdList.clear();
	...
	public void execute(BthMerInAccDtl inAccDtl) throws Exception {
		try{
			...
			gainsInfo = this.getMerGainsInfo(inAccDtl.getFundChannel());      //根据渠道查询分润比例信息Constans.IBANK_SYS_NO
			orgRelList = this.getMerOrgInfo(inAccDtl.getChlMerId());      //根据一级商户号查询商户机构信息
			...
			log.info("本行订单清分出现异常_[{}]",e.getMessage());
...
//	@Transactional(propagation = Propagation.REQUIRED,rollbackFor = {Exception.class})
			...
			MchtContInfo merSettleInfo, String cur, List<BthSetCapitalDetail> bthSetCapitalDetails)
	{
		...
		if (Constans.ORDER_TYPE_CONSUME.equals(settleInfo.getOrderType()))
		{
			...
			detail.setAccountType(merSettleInfo.getSettlAcctType());          // 结算账户类型0-本行1-他行
			}
			//2)反佣
			if(!IfspDataVerifyUtil.isBlank(settleInfo.getCommissionAmt()))
			{
				...
				if(commAmt.compareTo(ZERO) == 1)
				{
					...
					commDetail.setAccountType(commissionMer.getSettlAcctType());          // 结算账户类型0-本行1-他行
					
					//金额转换，分转换成元
					commDetail.setTranAmount(commDetail.getTranAmount().divide(ONE_HUNDRED));
					
//					bthSetCapitalDetailDao.insert(commDetail);
			...
			if(!IfspDataVerifyUtil.isBlank(settleInfo.getLogisFee())  && !"0".equals(settleInfo.getLogisFee()))
			{
				...
				if(logisAmt.compareTo(ZERO) == 1)
				{
					...
					else
					{
						...
						logisDetail.setAccountType(platMer.getSettlAcctType());          // 结算账户类型0-本行1-他行
						
					}
					//金额转换，分转换成元
					logisDetail.setTranAmount(logisDetail.getTranAmount().divide(ONE_HUNDRED));
//					bthSetCapitalDetailDao.insert(logisDetail);
...
//				bthSetCapitalDetailDao.insert(detail2);
...
//	@Transactional(propagation = Propagation.REQUIRED,rollbackFor = {Exception.class})
			...
			MchtContInfo merSettleInfo,MchtGainsInfo gainsInfo, List<BthSetCapitalDetail> bthSetCapitalDetails,ParternBaseInfo serviceInfo)
	{
		...
		BigDecimal merFeeAmt = new BigDecimal(settleInfo.getSetlFeeAmt());   //手续费金额
		//行内手续费=商户手续费+物流手续费
		if(settleInfo.getLogisFeeAmt() != null &&  !"0".equals(settleInfo.getLogisFeeAmt()))
		{
		...
		//收单机构分润比例
		if(gainsInfo.getCellectOrgDist() != null && gainsInfo.getCellectOrgDist().compareTo(CoreClearingExecutor.ZERO) ==1)
		{
		...
		//发卡机构分润比例
		if(gainsInfo.getSendOrgDist() != null && gainsInfo.getSendOrgDist().compareTo(CoreClearingExecutor.ZERO) ==1 )
		{
			totalRatio = totalRatio.add(gainsInfo.getSendOrgDist()); 
			ProfitsInfo gain = new ProfitsInfo();
			gain.setOrgType("03"); 
			if(IfspDataVerifyUtil.isBlank(settleInfo.getOpenBrc()))
			{
				gain.setProfitOrg(merInfo.getRpsOrgNo());
			}
			else
			{
				gain.setProfitOrg(settleInfo.getOpenBrc());
			}
			gain.setRatio(gainsInfo.getSendOrgDist());
			
			gains.add(gain);
		}
		//运营机构分润比例
		if(gainsInfo.getProxyOrgDist() != null && gainsInfo.getProxyOrgDist().compareTo(CoreClearingExecutor.ZERO) ==1 && (merInfo.getOpOrgNo()!= null))
		{
		...
		for(ProfitsInfo gain : gains)
		{
			...
			curOrgFee = merFeeAmt.multiply(actualRatio); // **结算金额精确到小数点后几位？？
			curOrgFee = curOrgFee.setScale(0, BigDecimal.ROUND_HALF_UP);
			
			
			
			//确保不多分手续费
			if(curTotalFee.add(curOrgFee).doubleValue() >= merFeeAmt.doubleValue())
			{
				curOrgFee = merFeeAmt.subtract(curTotalFee);
			}
			...
			switch (gain.getOrgType())
			{
				...
				case "02": // 电子银行中心（**运营机构？）
					if(serviceInfo!=null){
						...
						detail.setParternCode(serviceInfo.getParternCode());//服务商编号
					}else{
					...
					break;
				case "03": // 发卡机构
					detail.setEntryType(Constans.ENTRY_TYPE_FEE_GAINS_OPEN_ORG);    // 06-手续费分润（发卡行）
					detail.setInAccountNo(gain.getProfitAcc()); // 转入账户(商户结算账户)
					detail.setInAccoutOrg(gain.getProfitOrg()); // 转入商户机构号
					break;
			}
		...
		else if (Constans.ORDER_TYPE_RETURN.equals(settleInfo.getOrderType()))
		{
			...
			switch (gain.getOrgType())
			{
				...
				case "02": // 运营机构
					...
					break;
				case "03": // 发卡机构
					detail.setEntryType(Constans.ENTRY_TYPE_FEE_PAY_OPEN_ORG);    // 10-手续费支出（发卡行）
					detail.setOutAccountNo(gain.getProfitAcc()); // 转出账户(商户结算账户)
					detail.setOutAccoutOrg(gain.getProfitOrg()); // 转出商户机构号
					break;
			}
...
//			bthSetCapitalDetailDao.insert(detail);
	...
	private BthSetCapitalDetail initCapitalDetail(BthMerInAccDtl settleInfo, MchtBaseInfo mgtMerInfo, String cur)
	{
		...
		detail.setTranAmount(new BigDecimal(0));
	...
	public MchtGainsInfo getMerGainsInfo(String channelNo) {
		MchtGainsInfo gainsInfo = new MchtGainsInfo();
